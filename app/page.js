...
function Login(){
 ...
 return <div className="auth"><div className="authbox"><div className="logo">AVA <span>Alpha 0.3</span></div><h1>{mode==='login'?'Anmelden':'Verkäuferkonto erstellen'}</h1><p>Autohaus Vertriebs Assistent</p>
...
function Dashboard({session}){
 ...
 const [busy,setBusy]=useState(true),[selected,setSelected]=useState(null),[form,setForm]=useState({name:'',customer_number:'',phone:'',email:'',vehicle_interest:'',stage:'lead',notes:'',contract_end_date:'',ordered_at:'',delivered_at:''});
 ...
 function edit(c){setSelected(c);setForm({name:c.name||'',customer_number:c.customer_number||'',phone:c.phone||'',email:c.email||'',vehicle_interest:c.vehicle_interest||'',stage:c.stage||'lead',notes:c.notes||'',contract_end_date:c.contract_end_date||'',ordered_at:c.ordered_at||'',delivered_at:c.delivered_at||''});setShowForm(true)}
 function fresh(){setSelected(null);setForm({name:'',customer_number:'',phone:'',email:'',vehicle_interest:'',stage:'lead',notes:'',contract_end_date:'',ordered_at:'',delivered_at:''});setShowForm(true)}
 ...
 return <main className="shell">
  <aside><div className="logo">AVA <span>0.3</span></div>{['Heute','Kalender','Kunden','Historie','Team'].map(x=><button key={x} className={tab===x?'active':''} onClick={()=>setTab(x)}>{x}</button>)}<div className="asideBottom"><small>{session.user.email}</small><button onClick={()=>supabase.auth.signOut()}>Abmelden</button></div></aside>
